...
export function RegistrarShell({ children }) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F7F9FC] text-[#1D2939] flex flex-col font-sans">
      ...
      {/* Demo Sandbox Banner */}
      <div className="bg-[#FEF6EE] border-b border-[#F9DBAF] px-4 py-1.5 text-center text-xs font-mono text-[#B54708] flex items-center justify-center gap-2">
        <Info className="h-3.5 w-3.5 shrink-0 text-[#D92D20]" />
        <span>
          <strong>OFFICIAL SUB-REGISTRAR WORKSPACE:</strong> TitleLock Cadastral Adjudication Authority. Actions mutate local demo storage only.
        </span>
        <button
          onClick={handleReset}
      ...
      {isRestricted && (
        <div className="bg-[#FEF3F2] border-b border-[#FECDCA] px-4 py-2 text-center text-xs text-[#B42318] flex items-center justify-center gap-2">
          <ShieldAlert className="h-4 w-4 shrink-0 text-[#D92D20]" />
          <span>
            <strong>RESTRICTED ADMINISTRATIVE STATUS (SCN-16):</strong> Officer {user?.name} ({user?.badge}) operates under dual-role restrictions (CANNOT_AUDIT_OWN_ACTIONS, CANNOT_FINALIZE_OWN_TRANSFER). Final transfer execution is disabled.
          </span>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              <Link to="/registrar/dashboard" className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-base tracking-tight text-[#0B3A67]">Sub-Registrar</span>
                    <span className="rounded bg-[#0B3A67] text-white px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                      Adjudication Desk
                    </span>
                  </div>
